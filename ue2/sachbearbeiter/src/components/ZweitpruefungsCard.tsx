 ...
 *  - Component nutzt intern usePruefung (Singular) für das letzte
 *    Prüfprotokoll — keine letzteErstpruefung-Prop mehr.
 *  - Pflicht-Heuristik (Ablehnung + Dissens-Status) intern berechnet,
 *    spiegelt Backend pruefung-service._zweitpruefung_erforderlich.
 *  - useBekannteBearbeiter ist (noch) nicht restauriert → Vorschläge
 *    bleiben leer, Datalist trotzdem rendern für späteres Drop-in.
 */
...
import {
  usePruefung,
  type PruefBefund,
  type PruefprotokollRow,
} from "../hooks/usePruefung";
...
interface Props {
  antragId: string;
  /** Optional: Status des Antrags. Wird genutzt, um den Pflichtgrund-Text
   *  zu schärfen (z.B. „zweitpruefung_dissens" → Dissens-Hinweis). */
  antragStatus?: string;
  /** Wenn `false`, wird der KI-adversariell-Pfad ausgeblendet (Auswahl-Button
   *  + Trigger-Section). Mensch-Zweitprüfung bleibt vollständig nutzbar.
   *  Default `true` (UE3-Verhalten). UE2 übergibt `false`. */
  withKi?: boolean;
}

/** Trigger-Logik für die Pflicht-Zweitprüfung, gespiegelt aus dem Backend
 *  (pruefung-service main._zweitpruefung_erforderlich). Quelle der Wahrheit
 *  bleibt das Backend (es schaltet den Status). */
function istZweitpruefungPflichtig(
  letztePruefung: PruefprotokollRow | null,
): boolean {
...
function pflichtgrundFor(
  letztePruefung: PruefprotokollRow | null,
  status: string | undefined,
): string | undefined {
  if (status === "zweitpruefung_dissens")
    return "Erst- und Zweitprüfung im Dissens — bitte auflösen.";
...
export function ZweitpruefungsCard({ antragId, antragStatus, withKi = true }: Props) {
  const { session } = useSession();
  const email = session?.user?.email ?? null;
  const { latest: letzteErstpruefung } = usePruefung(antragId);
  const {
    zweitpruefung,
    upsert,
  ...
  const zweitpruefungPflicht = istZweitpruefungPflichtig(letzteErstpruefung);
  const pflichtgrund = pflichtgrundFor(letzteErstpruefung, antragStatus);
    ...
    <ZweitpruefungsBody
      ...
      onSave={(patch) =>
        upsert({
          id: zweitpruefung.id,
          rolle: "zweitpruefung",
          pruefer_typ: zweitpruefung.pruefer_typ,
          pruefer_id: zweitpruefung.pruefer_id,
          pruefer_modus: zweitpruefung.pruefer_modus,
          pruefprotokoll_id: zweitpruefung.pruefprotokoll_id,
          ...patch,
        })
      }
      onReset={async () => {
...
function ZweitpruefungsStart({
  ...
  antragId: string;
  email: string | null;
  /** Wenn false, wird der KI-Pfad komplett unterdrückt (Auswahl-Button +
   *  ki-Step ausgeblendet). Mensch-Optionen bleiben vollständig nutzbar. */
  withKi: boolean;
  kiRunning: boolean;
  ...
  // useBekannteBearbeiter ist (noch) nicht restauriert — Liste bleibt leer,
  // das Eingabefeld funktioniert ohne Vorschläge weiter.
  const bekannteEmails: string[] = [];
  ...
  if (mode === "select") {
    return (
      <div className="space-y-3">
        ...
        <div className={withKi ? "grid grid-cols-1 sm:grid-cols-3 gap-2" : "grid grid-cols-1 sm:grid-cols-2 gap-2"}>
          <Button
            variant="outline"
        ...
        <p className="text-xs text-slate-500 leading-relaxed">
          ...
          Person als die/der Erstprüfer:in.
          {withKi && (
            <> KI-Variante ergänzt oder ersetzt die Mensch-Prüfung, ist
            aber methodisch nicht das klassische Vier-Augen.</>
          )}
        </p>
      </div>
    );
...
function ZweitpruefungsBody({
  ...
  letzteErstpruefung: PruefprotokollRow | null;
  error: string | null;
  ...
  async function speichern(abschliessen: boolean) {
    ...
    try {
      await onSave({
        ...
        entscheidungs_vorschlag:
          (vorschlag || undefined) as EntscheidungsVorschlag | undefined,
        abschliessen,
      });
    } finally {
      setSaving(false);
      ...
      <CardContent className="space-y-4">
        ...
        {istAbgeschlossen && (
          <div className="text-xs bg-emerald-50 border border-emerald-200 px-2 py-1 rounded">
            ...
            Vorschlag:{" "}
            <strong>
              {vorschlag ? VORSCHLAG_LABELS[vorschlag as EntscheidungsVorschlag] : "—"}
            </strong>
          </div>
        )}
