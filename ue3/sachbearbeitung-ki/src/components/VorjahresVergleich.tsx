 ...
 *
 * Lädt /api/antrag/{id}/vergleich-vorjahr on-mount via useVergleichVorjahr.
 * Zeigt eine Tabelle der relevanten Änderungen — kritische und auffällige
 * zuerst. Wenn kein Vorjahres-Antrag existiert: dezenter Hinweis.
 */
...
import {
  useVergleichVorjahr,
  type VorjahresAenderung,
} from "../hooks/useVergleichVorjahr";
...
export function VorjahresVergleich({ antragId }: Props) {
  const { data, loading, error } = useVergleichVorjahr(antragId);
...
function AenderungZeile({ aenderung: a }: { aenderung: VorjahresAenderung }) {
  const farbe =
    a.schwere === "kritisch"
